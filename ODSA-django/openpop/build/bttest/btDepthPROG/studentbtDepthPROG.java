...
public class studentbtDepthPROG
{
 ...
 public static int modelbtDepth(BinNode rt) {
    ...
    int leftDepth = modelbtDepth(rt.left());
    int rightDepth = modelbtDepth(rt.right());
...
public static int btDepth(BinNode rt) {
  // Your code goes between these lines
if (rt == null) return 0; 
    else{
    int leftDepth =btDepth(rt.left());
    int rightDepth = btDepth(rt.right());
    return 1+ Math.max(leftDepth , rightDepth);
    }
