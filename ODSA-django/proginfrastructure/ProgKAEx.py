...
def assessprogkaex(
    ...
    feedback = [False, 'null', i + 1, studentfilename, 'class '
                + studentfilename]
    ...
    if progexType == 'pointers':
        ...
        answer.write(test)
        TestFilepart2 = open(filesPath + testfilenamep + 'part2.java',
                             'r')
    ...
    policyfile.write('permission java.io.FilePermission "'
                     + peruserFilesPath + 'output","read , write";')
    policyfile.write('permission java.io.FilePermission "'
                     + peruserFilesPath
                     + 'compilationerrors","read , write";')
    policyfile.write('permission java.io.FilePermission "'
                     + peruserFilesPath + 'runerrors","read , write";')
    policyfile.write('permission java.io.FilePermission "'
                     + peruserFilesPath + studentfilename
                     + '","read , write , execute";')
    policyfile.write('};')
    policyfile.close()
    ...
    proc1 = subprocess.Popen(' cd ' + peruserFilesPath + '; javac '
                             + studentfilename + ' 2> '
                             + peruserFilesPath
                             + 'compilationerrors.out ; java -Djava.security.manager -Djava.security.policy==newpolicy.policy student'
                              + testfilenamep + ' 2> '
                             + peruserFilesPath + 'runerrors.out',
    ...
    for num in range(0,5):
        ...
        if os.path.isfile(peruserFilesPath + 'compilationerrors.out'):
            syntaxErrorFile = open(peruserFilesPath
                                   + 'compilationerrors.out', 'r')
            feedback[0] = False
        ...
        if checkDefinedvar == 'True':
            if data.count(datatypes[0]) > 1 or any(x in data for x in
                    datatypes[1:len(datatypes)]):
                feedback[1] = \
                    ['Try Again! You should not declare any variables.']
                return (feedback, peruserFilesPath)
        ...
        if os.path.isfile(peruserFilesPath + 'runerrors.out'):
           ...
           # Check what is returned from the test : what is inside the success file
            ...
            for line in feedback[1]:
                print 'line' + line
                if 'at java.security.AccessControlContext.checkPermission' \
                    in line:
                    feedback[1] = \
                        ["Try Again! Your solution shouldn't write files to the disk."
                         ]
                    return (feedback, peruserFilesPath)
                elif 'Exception in thread "main" java.lang.StackOverflowError' \
                    in line:
                    feedback[1] = \
                        ['Try Again! Your solution leads to infinite recursion!'
                         ]
                    return (feedback, peruserFilesPath)
                elif 'NullPointerException' in line and progexType \
                    == 'binarytree':
                    feedback[1] = \
                  ...
                  # return feedback
                    ...
                    feedback[1] = \
                        ['Try Again! You probably have infinite recursion.']
                    ...
                    feedback[1] = ['Try Again! Your solution leads to '
                                   + line.replace('Exception in thread "main" java.lang.'
                                   , '')]
                    return (feedback, peruserFilesPath)
               ...
               # it is correct but..for binary trees check they are doing it in the effecient way
                ...
                if 'Well Done' in line:  # static code analysis
                    if progexType == 'BTRecurTutor':
                        (efficient, btineffFB) = btcheckEfficientCode(data,
                                testfilenamep)
                        if efficient == True:
                            feedback[0] = True
                            return (feedback, peruserFilesPath)
                        ...
                        (efficient, pntrineffFB) = \
                            pntrcheckEfficientCode(data, testfilenamep)
                        if efficient == True:
                            feedback[0] = True
                            return (feedback, peruserFilesPath)
                        else:
    ...
    feedback[1] = ['Server is busy right now, please try again in 10 mins!']
    return (feedback, peruserFilesPath)
